...
**约束链逐步定量。** (i) $A_{cu}=I/J=20/4=5.000\ \mathrm{mm^2}$（精确值，`data/q2_q3_baseline.json: design.a_cu_exact_mm2`）。(ii) 取 $N=127=3k(k{+}1)+1$（$k{=}6$），即完整六角环 0–6、丝数 $1{+}6{+}12{+}18{+}24{+}30{+}36$（`data/q2_ring_currents.csv: count` 列）——完整环保证 6 重旋转对称下同环丝严格等价，电流失衡只剩径向结构，环统计才有确定含义（`geo/strand_packing.py`；`findings.md` H+7）。(iii) 反解丝径 $d=2\sqrt{A_{cu}/(N\pi)}=0.2239\ \mathrm{mm}$，$\beta=d/\delta=1.515$（$\delta=147.77\ \mu$m），孤立丝精确 Bessel 趋肤因子 $F(\beta)=1.00682$——单丝自趋肤代价仅 $+0.7\%$，可忽略（`data/q2_q3_baseline.json: skin`）。(iv) 束外径 $OD=2.969\ \mathrm{mm}$，铜填充率 $72.2\%$（真实 Litz 区间；六方密排上限 $90.7\%$），$R_{dc}=\rho/A_{cu}=3.448\ \mathrm{m\Omega/m}$（`data/q2_results.json`、`data/q2_tradeoff.json: design_point`、`geo/strand_packing.py`）。

**丝径权衡扫描（36 点，$d\in[0.06,0.40]$ mm，$A_{cu}$ 恒 5 mm²，图 Q2-4、`data/q2_tradeoff.csv`）给出反直觉结论：未绞合束的比值随 $d$ 变细不降反升**——粗端 $N{=}37$/$d{=}0.415$ mm 为 3.43，设计点 4.17，$d\le0.171$ mm 进入 4.53–4.56 平台，最细 $N{=}1801$ 时 4.5326，恰收敛于等铜面积实心导体的精确值 4.5295（`data/q2_tradeoff.json: trend`）。机理：电流挤占由**束径**决定（全扫描 $OD\in[2.757,2.969]$ mm，$D/\delta\approx19$–20，由同表 OD 列与 $\delta$ 换算），与丝径无关；失衡指标恒在 3.19–4.63 倍（同表 `imbalance_L1` 列）。故 $d$ 的选型依据**不是「越细越好」，而是保证 $F(\beta)\approx1$**。

**弃选说明。** $d=0.2$ mm 时 $N_{exact}=159.2$ 非完整六角环数（邻近完整环需 $N{=}169$、$d{=}0.194$ mm），残缺外环破坏对称性（`data/q2_tradeoff.csv` $d{=}0.2$ 行；`findings.md` H+7）；工业丝径 0.05–0.07 mm 对应 $N=1261$–1801（`data/q2_tradeoff.csv` 前两行），逐丝多导体 FEM 规模不可解，且扫描已证明无益（该端 $F=1.00003$–1.00007，比值停在 4.53 平台）。

**诚实性声明（L1 口径）。** 扫描用 L1 丝状阻抗模型，绝对值系统性偏低约 10%（设计点 4.171 vs FEM 4.65–4.68），且偏差随 $\beta$ 增大——粗端（$N{=}37/61$，$\beta>2.2$）仅作趋势参考，**不得**读作「粗丝更好」（`data/q2_tradeoff.json: notes、argmin_ratio_L1.caveat`）。

![图 Q2-4](figures/q2_fig4_tradeoff.png)

**图 Q2-4**　(a) 丝径扫描：未绞合比值随 $d$ 变细不降反升，收敛到实心值 4.5295；(b) 单丝趋肤因子 $F(\beta)$。数据：`data/q2_tradeoff.csv`（L1 口径，偏差声明见正文）。
...
**头条：$R_{ac}/R_{dc}=4.6527$**（refine=3 raw，$R_{ac}=16.044\ \mathrm{m\Omega/m}$），Richardson 外推 $\approx4.654$（`data/q2_convergence.csv`；`findings.md` H+10）。
...
未绞合束比实心**差 $+2.75\%$**（Richardson 口径 4.654/4.5295；raw 口径 +2.72%，`findings.md` H+10）。
...
环 6 两法偏差 $+3.1\%$；内外丝电流相位差约 60°（+46° vs −13°，同文件）。注：图 Q2-2 蓝柱按半径聚类，最外组为外环 18 根靠角丝（4.0×）——六角环并非等半径圆，两种聚类口径并存，数值均出自同一 CSV（`q2_litz/impedance_model.py: ring_radii` 注记）。
...
**图 Q2-2**　径向失衡：中心 1.1% → 外层 400%（FEM vs L1）。数据：`data/q2_strand_currents.csv`（聚类口径见表 Q2-4 注）。

![图 Q2-3](figures/q2_fig3_comparison.png)

**图 Q2-3**　等铜截面三方案：未绞合束（4.683，refine=1 口径）不优于实心线（4.529）。数据：`data/q2_results.json`、`data/q2_q3_baseline.json`；收敛头条 4.653 见表 Q2-3。

## 5　任务四：讨论

**(1) 各丝电流均匀吗？——严重不均，且是系统性的径向梯度。** $\eta_I=(|I|_{max}-|I|_{mean})/|I|_{mean}\approx438\%$（`data/q2_strand_currents.csv`；refine=3 为 440.8%）；中心丝近乎不导电（0.010×），环电流从中心 0.011×单调升到最外环 3.37×、角丝 4.0×（表 Q2-4）。机理为**电感性再分配**：并联丝共享同一 $v$，外层丝链通的内部磁通少 → 自感小 → 阻抗低 → 抢流；等效于束级「空管效应」——束半径 1.484 mm 大于等面积实心的 1.262 mm（$D=2.523$ mm 之半），且外缘填充仅 72%，电流被推到更大半径、有效载流层反而更薄。这正是未绞合束比实心差 +2.75% 的原因（`findings.md` H+7/H+10）。
...
**(3) 引出问题三。** 丝径扫描证明：未绞合（含单绞向绞合）束趋同实心值（4.53–4.65），丝径只负责 $F(\beta)\approx1$；要把 3.2–4.6× 的径向失衡抹平、逼近 1.0068 的理想换位下限，唯一出路是让丝**真实交换径向位置**——径向换位/三维编织，即问题三的设计对象（`data/q2_tradeoff.json: headline_finding`）。
...
| 头条 4.6527/4.654、实心 4.5295/4.5310、束差 +2.75% | `data/q2_convergence.csv`、`data/q2_q3_baseline.json`、`findings.md` H+10 |
| 逐丝/环电流、$\eta_I$、环 6 FEM↔L1 +3.1% | `data/q2_strand_currents.csv`、`data/q2_ring_currents.csv`、图 Q2-1/Q2-2、`q2_litz/impedance_model.py` |
...
| 几何/网格/求解器与复现 | `geo/strand_packing.py`、`sim/gmsh_mesh.py`、`q2_litz/solve_bundle.py`、`data/q2_bundle_r1/2/3.msh`、`data/q2_solid_eq_r1/2.msh`；复现顺序 `./.venv/bin/python geo/strand_packing.py` → `q2_litz/solve_bundle.py` → `refine3.py` → `tradeoff_scan.py` → `make_figs.py` |
