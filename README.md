...
但**排布几何本身就是设计变量**：同样的 331 丝 / 0.14 mm / 同铜面积，只把六角密排换成同心圆环（环内方位对称），环内电流失衡即压到 0.4% 以内（最大 0.36%），Rac/Rdc = **4.403，反超等面积实心 3.6%**（队友 COMSOL 4.4033；我方 skfem 独立复现 4.408，边界收敛后差 0.12%）。不过距单丝理想值 1.001 仍有 4.4 倍空间——这剩余部分只能靠**真实径向换位**消除，正是 Q3/Q4 的战场。
