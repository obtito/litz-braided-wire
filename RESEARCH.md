...
> 71 项经 7 批对抗复审（58 确认 / 13 部分 / 0 错误 / 0 编造）；修正已就地标注（【审计勘误】或等效说明）。详见《研究原始汇总.md》顶部审计层。
...
| # | 决策 | 依据 |
|---|------|------|
| 1 | **NGSolve**（wheel 实测装通；**peer-review 角色修正：2D 生产路线已用 scikit-fem**——ngsolve wheel 的 gmsh 导入器损坏，NGSolve 留作 L3/3D 备选） | PyPI 实测 v6.2.2607 有 cp310–312 macOS wheel；复数时谐 H(curl) 边单元原生 MQS 涡流问题；全 Python 驱动，Q3 优化循环在进程内秒级重解 |
...
| 6 | Q2 设计：**双层设计**——「分析最优设计」d=0.05–0.07 mm（Sullivan 准则，N≈1300–2500，只做解析评估）+「仿真可解设计」d=0.2239 mm、N=127（完整六角环，β=1.515，F=1.0068）【原 d=0.2/N=159 已迭代：非完整六角环→束不对称，findings H+7】 | Sullivan & Zhang APEC2014（全文已读）+ 自算锚点 |
...
- **δ(200 kHz, Cu) = 147.77 μm**；扫频锚点 Rac/Rdc（2 mm 实心线，精确 Bessel 解）：
  50 k→1.966；100 k→2.662；**200 k→3.647**；500 k→5.609；1 M→7.822。L_int(200 kHz)=14.76 nH/m（DC 50 nH/m）。**这就是 Q1 仿真的收敛目标**。
...
- F(x) = (x/2)·[ber·bei′ − ber′·bei]/(ber′²+bei′²)（趋肤，F→1）——Umetani (10)
- K(x) = **−x·[ber₂(x)·ber′(x) + bei₂(x)·bei′(x)]/(ber²+bei²)**（邻近；ber₂+i·bei₂ = J₂(x·e^{+3iπ/4}) 为**二阶** Kelvin）——Umetani (11)【第三次修正：H+9 的 +2(ber·bei′−bei·ber′) 形式数值上 ≈ −⅓ 真值；本形式经解析 Bessel 边值解（7 位）与独立 FD（1.7%）双验证；scipy 无 ber₂，用 `scipy.special.jv(2, x·exp(3jπ/4))` 组合】
- x = γ_s = a_s·√(ωμ₀σ) = √2·a_s/δ（半径制；**注意与 β=d/δ 差 √2/2 倍，论文统一用 γ_s 并声明**）；设计点 d=0.2248mm → γ_s=1.0756，F=1.00693，K=0.08057
- ⚠️ Sullivan COMPEL (13) 的低频邻近系数与 Umetani (7) 差 2 倍——**裁决：采 Umetani**（我方数值验证支持）；引用 COMPEL 时注明口径差。
用于 Q2 丝径权衡曲线和一切交叉验证。
...
- 验证图配方：FEM |J(r)| vs J₀(kr)/J₀(ka) 曲线叠加；J(r) e 折叠长度 vs δ；L_int 50→14.76 nH/m 趋势。
...
- **Sullivan & Zhang APEC2014「Simplified Design Method for Litz Wire」**（免费 PDF ryz.ece.illinois.edu + elektrisola.com 镜像）：
  - 推荐丝数 n_e = k·δ²·**b**/N_s（b 在**分子**；k=√(192(F_R−1))/(πd_s³)（由 F_R 代数导出，非 Table I 原文），按丝径成本优化的列表常数，Table I，130–115000 mm⁻³）【审计勘误：原转录 k=1.0/1.33/2.0 构造系数系误记，已删】；
  - 首级最多丝数 n₁,ₘₐₓ = 4δ²/d_s²；每级 4–5 束；
  - 好设计 d_s ≤ δ（典型取 δ/4 或更小）；近邻-only 因子 F_R = 1+(πnN_s)²d_s⁶/(192δ⁴b²)；
  - 圆柱均匀场涡流损 P = πℓd⁴/(64ρ)(dB/dt)² → 损耗因子 ∝ n²d_s⁶。
- **Umetani et al. TIA 2021**（IEEE TIA 57(3), DOI 10.1109/TIA.2021.3063993，免费 PDF 已读）：多级绞合全解析模型（Kelvin F/K 闭式，论证变量 **γ_s=(d_s/2)·√(ωμ₀σ)=d_s·√(πfμ₀σ/2)**，半径制；m 为长度比因子乘在束电阻上，ρ_eff 见其式(13)【审计勘误：原 ρ/m 简写不准】），**式(49)-(51)(56)(57) 给出「忽略束级邻近效应所需的最小线长」**（= 给定线长下的最大捻距上界、半整数最坏对齐情形；**引用语义：勿称「最小捻距」**；1/3 电流 ↔ <10% 损耗标定出自原文 XII 页）——Q3/Q4 切片法有效性的文献判据。其参考文献表钉死了整个学派谱系（Dowell1966→Ferreira1992/94→Sullivan1999/2001→Nan&Sullivan→Rosskopf2014→Sullivan&Zhang2014）。
- **Zhang/White/Kassakian MIT APEC2014**（免费 PDF 已读）：P = F(f)·I²R_dc + G(f)·|H|² 分解 + PEEC 细丝法（比 FEM 快数量级，1 MHz 离散误差 <0.1%）；其捻距敏感性扫描就是 Q3 模板。
- **本题设计数（自算）**：A_cu=5 mm²（J=4 A/mm²）；d=0.05 mm→N=2546（n₁max=35，3 级）；d=0.071→N≈1265；d=0.1→N=637；**d=0.2→N=159（初版方案，已迭代）**；d=0.25→N=102。**最终定型 N=127/d=0.2239mm（完整六角环，β=1.515，F=1.0068）——findings H+7 / data/q2_q3_baseline.json**
...
| ③ 无实测基准/行业数值 | 填充率 0.30–0.60（含漆膜）；Roebel 换位长/丝宽比 22–67（Goldacker 六缆表，审计层#4）；Sullivan 论文内含实测表；MTC Litz 编织实例；其余在 Q2 时再查 Elektrisola/New England Wire 数据表 |
