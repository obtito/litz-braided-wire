...
def ring_radii(centers, spacing):
    ...
    q = (math.sqrt(3) / 3 * centers[:, 0] - centers[:, 1] / 3) / spacing
    r = (2.0 / 3.0 * centers[:, 1]) / spacing
    labels = np.round((np.abs(q) + np.abs(r) + np.abs(q + r)) / 2).astype(int)
...
def twisted(centers, r_s, pitch, r_air, f=F0, i_tot=I_RMS, with_prox=True):
    """规则绞合（方位角平均环模型）。pitch=绞距[m]。返回指标字典。"""
    spacing = 2 * r_s * 1.02
    ...
    ohm_loss = np.sum(rdc_str * F * len_fac * (Ik / counts) ** 2 * counts)  # 每丝 |I_i|² 同环相等
    if with_prox:
        ...
        prox = np.sum([counts[k] * prox_loss_per_strand(2 * r_s, h_ring[k], f)
                       for k in range(n_rings)])
    else:
        prox = 0.0
    p_tot = ohm_loss + prox
    ...
    return dict(ring_labels=labels, ring_r=ring_r, counts=counts,
                ...
                eta_I=(per_strand_i.max() - i_tot / len(centers)) / (i_tot / len(centers)))
...
if __name__ == "__main__":
    ...
    # --- 规则绞合（绞距 30 mm） ---
    tw = twisted(centers, r_s, pitch=30e-3, r_air=r_air)
    print(f"\n规则绞合（P=30mm）：R_ac/R_dc = {tw['ratio']:.4f}"
          f"（欧姆 {tw['p_ohm']:.2f} W + 邻近 {tw['p_prox']:.2f} W）"
          f"  η_I = {tw['eta_I']*100:.1f}%")
    print("环电流相对均值：", np.round((tw['per_strand_i'] / imean), 3).tolist())
    json.dump({k: (v.tolist() if isinstance(v, np.ndarray) else v)
               for k, v in tw.items() if k != "ring_labels"},
              open("data/q2_twisted_l1.json", "w"), ensure_ascii=False, indent=1, default=float)
    print("-> data/q2_twisted_l1.json")
