...
if __name__ == "__main__":
    ...
    # 内嵌放大框：表层 r∈[0.86,1.0] mm——三法在视觉上可分辨（全图重合的诚实呈现）
    axin = ax.inset_axes([0.42, 0.45, 0.30, 0.28])
    m_in = (r_fem[2:] >= 0.86e-3)
    axin.plot(r_fem[2:][m_in] * 1e3, jr_fem[2:][m_in] / 1e6, color=C_FEM, lw=1.6)
    m_fd = (r_fd >= 0.86e-3)
    axin.plot(r_fd[m_fd] * 1e3, jr_fd[m_fd] / 1e6, color=C_FD, lw=1.2, ls="--")
    axin.plot(r_fd[m_fd] * 1e3, jr_exact[m_fd] / 1e6, color=C_EX, lw=1.0, ls=":")
    axin.set_xlim(0.86, 1.005)
    axin.tick_params(labelsize=7)
    axin.set_title("表层放大", fontsize=8, color=INK)
    axin.grid(alpha=0.25, lw=0.4)
    ax.indicate_inset_zoom(axin, edgecolor="#8b949e", lw=0.7)
    ...
    ref_x = np.array([64, 2048])
    ax.plot(ref_x, fd_err[2] * (ref_x / 64.0) ** -2, ":", color=INK, lw=0.9, alpha=0.5)
    ax.text(700, fd_err[2] * (700 / 64.0) ** -2 * 1.6, "二阶 O(h²) 参考", fontsize=8,
            color=INK, rotation=-22, alpha=0.7)
    ...
    axes[1].set_title("(b) 趋肤深度 δ(f) ∝ f^(−1/2)", fontsize=10)
    for axx in axes:
        axx.set_xticks(fs / 1e3)
        axx.set_xticklabels([str(int(f / 1e3)) for f in fs])
